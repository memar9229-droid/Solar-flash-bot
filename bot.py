...
PAYMENT_METHODS = [
    {"id":"sol",      "name":"SOL  — Solana Network",  "emoji":"◎"},
    {"id":"btc",      "name":"BTC  — Bitcoin Network",  "emoji":"₿"},
    {"id":"usdttrc20","name":"USDT — TRC20 (Tron)",     "emoji":"💵"},
    {"id":"usdcarbitrum", "name":"USDC — Arbitrum Network", "emoji":"💰"},
]
...
def get_tier(uid):
    if uid in ADMIN_IDS: return "alpha"
...
async def create_payment(uid: int, plan: str, currency: str) -> dict | None:
    ...
    try:
        async with aiohttp.ClientSession() as s:
            ...
            if RAILWAY_URL and isinstance(RAILWAY_URL, str) and len(RAILWAY_URL) > 5:
                payload["ipn_callback_url"] = f"https://{RAILWAY_URL.strip().replace('https://','').replace('http://','')}/payment/webhook"
            log.info(f"Creating payment: plan={plan} currency={currency} uid={uid}")
            async with s.post(
                f"{NP_BASE}/payment",
                ...
                log.info(f"NOWPayments response: status={r.status} body={resp_text[:200]}")
                if r.status == 201:
                    ...
                    log.error(f"NOWPayments create error: {r.status} {resp_text}")
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # PLUS plan selected
        ...
        await q.edit_message_text(
            f"🔥 Solar Flash PLUS\n\n"
            f"${p['price_usd']}/mo — {p['tag']}\n\n"
            f"{p['description']}\n\n"
            f"Future price: {p['future_price']}\n\n"
    ...
    # Payment method chosen — create invoice
        ...
        if payment:
            ...
            text = (
                ...
                f"2. Send exactly {amount} {curr}\n"
                f"3. Click the button below after sending\n\n"
                f"Payment verified automatically upon blockchain confirmation.\n"
                f"Powered by NOWPayments"
            )
            await q.edit_message_text(
                text,
                parse_mode=None,
...
async def cmd_upgrade(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    if is_alpha(uid):
        await update.message.reply_text("☀️ You are already on ALPHA — maximum tier!", parse_mode=None)
        return
    await update.message.reply_text(
        "⚡ *SOLAR FLASH UPGRADE*\n\n"
        ...
        "Accepted: SOL, BTC, USDT (TRC20), USDC (Arbitrum)",
        parse_mode=None,
        reply_markup=kb_plans(),
    )
...
async def cmd_admin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) < 2:
        await update.message.reply_text(
            "Admin commands:\n"
            "/admin set <user_id> <free|plus|alpha>\n"
    ...
    if action == "set" and len(args) == 3:
        try:
            ...
            target_tier = args[2].lower()
            if target_tier not in ("free","plus","alpha"):
                await update.message.reply_text("❌ Tier must be: free, plus, alpha")
                return
            u = get_user(target_uid)
            u["tier"]       = target_tier
            u["tier_since"] = time.time()
            await update.message.reply_text(
                f"✅ User {target_uid} set to {target_tier.upper()}\n"
                f"Tier active immediately."
            )
            log.info(f"Admin {uid} set user {target_uid} to {target_tier}")
        except ValueError:
        ...
        if not USER_DATA:
            ...
            return
        lines = [f"Total users: {len(USER_DATA)}\n"]
        for u_id, u_data in list(USER_DATA.items())[-20:]:
            lines.append(f"• {u_id}: {u_data.get('tier','free').upper()} | XP:{u_data.get('xp',0)} | Scans:{u_data.get('total_scans',0)}")
        await update.message.reply_text("\n".join(lines))
    elif action == "stats":
        total   = len(USER_DATA)
        plus    = sum(1 for u in USER_DATA.values() if u.get("tier")=="plus")
        alpha   = sum(1 for u in USER_DATA.values() if u.get("tier")=="alpha")
        free    = total - plus - alpha
        ...
        await update.message.reply_text(
            ...
            f"PLUS:         {plus}\n"
            f"ALPHA:        {alpha}\n"
            f"Pending pays: {pending}"
        )
    else:
        await update.message.reply_text("Unknown action. Use: set / list / stats")
...
async def cmd_signals(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_plus(uid):
        await update.message.reply_text(
            ...
            "Curated trading intelligence and market alerts.\n\n"
            "Upgrade to PLUS: $9.99/mo - /upgrade",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🔥 Upgrade PLUS", callback_data="upgrade_plus")
            ]])
        )
        return
    await update.message.reply_text(
        "📡 PREMIUM SIGNALS\n\nSignals rolling out in Phase 2B. Stay tuned."
    )
...
async def cmd_whales(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_plus(uid):
        await update.message.reply_text(
            ...
            f"Upgrade to PLUS: $9.99/mo — /upgrade",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🔥 Upgrade PLUS", callback_data="upgrade_plus")
            ]])
        )
        return
    await update.message.reply_text(
        "🐋 WHALE INTELLIGENCE\n\nWhale tracking active in Phase 2B. ⚡"
    )
...
def main():
    try:
        ...
        app.add_handler(CommandHandler("alerts",  cmd_signals))
        app.add_handler(CommandHandler("whales",  cmd_whales))
        app.add_handler(CommandHandler("stealth", cmd_whales))
