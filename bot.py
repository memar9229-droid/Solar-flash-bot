...
log.info("=== Solar Signal Bot Starting ===")
log.info(f"BOT_TOKEN present: {bool(BOT_TOKEN)}")
log.info(f"BOT_TOKEN length:  {len(BOT_TOKEN)}")
log.info(f"HELIUS_KEY present:{bool(HELIUS_KEY)}")

if not BOT_TOKEN:
    log.critical("FATAL: BOT_TOKEN is missing. Set it in Railway Variables.")
    sys.exit(1)

if not HELIUS_KEY:
    log.warning("HELIUS_KEY is missing. On-chain data will be limited.")
...
async def analyze(mint):
    ...
    if mint_info_raw:
        mint_info = (
            ((mint_info_raw.get("value") or {}).get("data") or {})
            .get("parsed", {})
            .get("info", {})
        )
    ...
    unique_w  = (be or {}).get("uniqueWallet24h")

    # Safety
    safety = 100
    if mint_auth:   safety -= 30
    if freeze_auth: safety -= 20
    if not lp_burned: safety -= 35
    ...
    whale = 0
    if top1 > 50:   whale += 60
    elif top1 > 25: whale += 35
    elif top1 > 10: whale += 15
    if top10 > 70:  whale += 30
    ...
    community = 30
    h_count   = len(holders)
    if h_count >= 2000:   community += 40
    elif h_count >= 500:  community += 25
    elif h_count >= 100:  community += 10
    else:                 community -= 10
    if has_tw:  community += 10
    if has_tg:  community += 12
    if has_web: community += 8
    community = max(0, min(100, community))

    # Narrative heat
    heat = 20
    if chg24:
        if chg24 > 200:   heat += 40
        elif chg24 > 50:  heat += 25
        elif chg24 > 10:  heat += 12
        elif chg24 < -50: heat -= 15
    if vol24:
        if vol24 > 500000:   heat += 25
        elif vol24 > 100000: heat += 15
        elif vol24 > 10000:  heat += 8
    total_tx = buys + sells
    if total_tx > 0 and buys > sells * 1.5: heat += 10
    heat = max(0, min(100, heat))

    # Scam
    scam = 0
    if mint_auth:    scam += 25
    if freeze_auth:  scam += 20
    if not lp_burned: scam += 30
    if top1 > 30:    scam += 15
    scam = min(100, scam)
    ...
    flags, goods = [], []
    if mint_auth:    flags.append("⚠️ Mint authority NOT revoked — supply can be inflated")
    else:            goods.append("✅ Mint authority revoked — supply fixed")
    if freeze_auth:  flags.append("⚠️ Freeze authority active — wallets can be frozen")
    else:            goods.append("✅ Freeze authority revoked")
    if lp_burned:    goods.append("🔥 Liquidity BURNED — rug pull impossible")
    else:            flags.append("🚨 Liquidity NOT secured — rug pull risk OPEN")
    if top1 > 50:    flags.append(f"🚨 Top wallet: {top1:.1f}% — extreme concentration")
    elif top1 > 25:  flags.append(f"⚠️ Top wallet: {top1:.1f}% — high whale risk")
    else:            goods.append(f"✅ Top wallet: {top1:.1f}% — healthy distribution")
    if top10 > 70:   flags.append(f"🚨 Top 10 hold {top10:.1f}% — cartel clustering")
    else:            goods.append(f"✅ Top 10 hold {top10:.1f}% — distributed")
    if has_tw:       goods.append("✅ X/Twitter confirmed")
    if has_tg:       goods.append("✅ Telegram community active")
    if has_web:      goods.append("✅ Website present")
    ...
    if solar >= 80:
        ...
        ai = f"Solar Signal flags elevated risk for ${symbol}. Structural safety is compromised. Proceed with strict caution."
    else:
    ...
    soc = []
    if has_tw:  soc.append("𝕏")
    if has_tg:  soc.append("TG")
    if has_web: soc.append("🌐")
...
async def run_analysis(mint, full, update):
    ...
    try:
        ...
        await msg.edit_text(
            "❌ *Could not analyze.* Check the address and try again.",
            parse_mode=ParseMode.MARKDOWN,
        )
...
async def cmd_help(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "• LP burn status\n"
        "• Whale concentration & dev wallet\n"
...
async def cmd_analyze(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not ctx.args:
        await update.message.reply_text(
            "Usage: `/analyze <address>`", parse_mode=ParseMode.MARKDOWN
        )
        return
...
async def cmd_score(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not ctx.args:
        await update.message.reply_text(
            "Usage: `/score <address>`", parse_mode=ParseMode.MARKDOWN
        )
        return
...
def main():
    try:
        ...
        log.info("⊙ Solar Signal Bot initialized successfully.")
